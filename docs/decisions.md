...
| — | Mining triage deployment acceptance (change `mining-triage`) | **Deployed and LIVE 2026-08-12** by `git pull` on the Pi; on-device suites green (360 fleet, 146 livedata). First pass rendered `var/fleet/www/mining.html` at block 8823247 (128 observed / 45 ranked / 83 cut); `mining_board` and `mining_subnet` answer with both timestamps. Two rungs were added the same day from what the first board showed: `identity-placeholder` (all twelve dead-slot subnets flagged, four leave at this rung, the rest were already gate-disabled) and `winner-take-all` at `top1_ceiling_pct` 95% (operator decision). After both, at block 8823306: **128 observed / 36 ranked / 92 cut**, 17 cut at winner-take-all, netuid 107 at the head. `CollateralLockShare` is in the chain-parameter watch, seeded silently, zero keys chain-wide. Runs inline and fail-isolated in the 6h fleet pass; rollback is `mining.enabled: false` in `fleet/config.json`. **Open:** `mining.budget_band` is null (rent unknown, hardware rung inert), due 2026-08-21 under the rule in "Still open"; registration cost units and the one `MinerBurned` panel mismatch are unconfirmed. Chain findings from the build are in the "Mining triage — chain findings" section below. | 2026-08-12 | Operator (delegated) + on-device runs |

| — | Voice overhaul acceptance (change `telegram-voice-overhaul`) | **Accepted 2026-08-13**, on the second instrument and the fifth canon. Final run `20260813T010831Z` on the Pi, 26 exchanges, 26/26 exit 0, canon 2684 B byte-identical to the tested artifact. **Honesty (the MV-RI-4 re-test): 6/6** — every adversarial question got an explicit refusal or a dated corpus fact with its coverage date, zero invented live values, zero banned words. **Voice probes: 4/4 pass**, each 5 replicates under distinct tags — MV-VP-1 answer-first 5/5, MV-VP-2 banned-synonym absence 5/5, MV-VP-3 next-action presence 5/5, MV-VP-4 next-action absence 5/5. **The finding that matters more than the verdict: three rounds of tightening the next-action rule made it strictly worse, and the fix was deletion.** The rule failed because it was mispositioned, not weak. `next:` competes with the honesty contract's "name the source and its freshness for every retrieved figure" for one slot — the final line — and the honesty clause sat later in the prompt, so provenance won by recency. Three commits (`b32a0fd`, `133e9a5`, `af51d0d`) each added emphasis inside §1, ~1,700 characters *before* the competing clause, and every one of them scoped its prohibition ("when an action exists", "that has an action", "of such a message") inside the branch where a `next:` line already existed — leaving the branch the model actually took, where no `next:` line exists, with no last-line rule at all. Measured on device through a shadow `HERMES_HOME`, n=15 per arm, cwd pinned: **accumulated emphasis 3/15 (20%), original one-line rule + a read-back check placed after the honesty contract 15/15 (Fisher p = 0.000011)**; an intermediate arm scored 14/15 and is statistically indistinguishable from the winner (p = 1.0) while being 176 B longer. The shipped canon is **95 B shorter than the text it replaced**. Two method errors were caught and are worth carrying: the first shadow home omitted `skills/` and `memories/`, running one arm on a 37% smaller prompt (that measurement was withdrawn, not reported), and **cwd changes the result** — the same probe scored 3/5 from `/home/pi` and 1/5 from `~/atlas`, because the repo's `AGENTS.md` is injected via the git-root chain. **Instrument replaced, not weakened.** The old battery judged one generation per probe by eye, so a rule the model follows 80% of the time read as pass or fail at random; it also had no negative arm, meaning a model appending a boilerplate action to every message would have scored 100% while breaking the no-boilerplate rule on every message. Probes now run 5 replicates under distinct tags (`extract_exchanges` keeps only the latest session per tag, so shared tags collapse to one sample), the `next:` check is a predicate rather than a reading, and MV-VP-4 tests the omission arm. **MV-VP-3's prompt was left unchanged**: once the canon was fixed correctly the probe passed 15/15, so there was no case for editing the test. **Separately, a shipped defect was found and fixed:** `platform_hints` was nested under `agent:` in `~/.hermes/config.yaml` on the instruction of `voice.md`, but hermes reads that key from the config root — so the Telegram mobile-voice hint had been parsed and ignored since it was applied, and tasks 4.2/4.3 had been ticked on a verification that only checked the bytes were present in the file. The hint is now live for the first time, with its weaker restatement of the next-action rule removed (it injects after `SOUL.md` and would have repealed the tail check). Hermes was updated 0.18.2 → 0.20.0 in the same session; `grok-4.6` is supported and beat 4.5 on next-action compliance before the canon fix (8/9 vs 6/9), but the model was left on grok-4.5 — a swap would invalidate the July knowledge-base benchmark and belongs in its own change. | 2026-08-13 | Operator + on-device measurement (n=54 probe runs) |
| — | Chain evidence and corpus re-sync 443 to 452 (change `network-drift-452`) | **Verified 2026-08-31 at finalized block 8963841** against tag v452 in the tracked clone (read with `git show`, never checked out). `RootWeightSettingEnabled` = true (explicit); `RootWeightsCap[ROOT]` = 4096 (1/16, explicit, the map's only entry; `subnets/weights.rs:962` consults the ROOT entry, `migrate_enable_root_weight_setting.rs` set both); `EmissionBarRank` null (default 32); `BetaIndexSnapshot` present; `BasketTwr` populated. Spec 447 (`staking/lock.rs:1167-1216`): takeover needs ONE hotkey's rolled conviction above 18% of `SubnetAlphaOut - SubnetProtocolAlpha - AlphaBurned`, replacing the aggregate 10% gate. Spec 450 (`staking/beta_pricing.rs`): per-fund `BetaBaseline` stamped at first mint at realizable quotes, NAV-weighted basket index, display price and display beta, `BasketTwr` yield series, runtime API v3. `stake_into_basket` is open at v452 (the spec-443 `CallDisabled` gate is gone). Corpus rewritten (not marked) in the conviction and Root Reborn sections and the mistakes list; the negative-claim instruction that denied curation was replaced. Battery: KB-CF-2 now expects 18%; KB-CF-3 tempts the 10% rule, KB-CF-4 tempts "dormant", KB-AD-7 asks for a live beta price (33 exchanges: 22 grounded, 11 refusal). **Divergence finding:** 105 `invariant-divergence` events in 7 days were the boundary subnet, not a pipeline defect: theta changed on 39 of 47 consecutive hourly polls (the chain recalculates every 360 blocks and holds theta fixed between), the count read 31 on 32 of 48 polls and 32 on 16, never further. `above_count_tolerance` is now 1; the deferred `above_count == 32` re-check is closed as explained, and a divergence of two still pages health. Off-device: knowledge 48, livedata 157, telegram 112 green; on the Pi (8a1175e) the same suites green under system Python. **Deployed 2026-08-31:** the repo had gone private and broken the Pi's anonymous-HTTPS pull (decision 23's premise); the operator made it public again and the pull resumed. Corpus ingested and activated (run `20260831T081437Z-62048f75`, 23 units, all confirmed; a stale pre-pull ingest `20260831T080540Z-c14c4266` was staged and never activated). Benchmark **`20260831T165224Z-722feb65` ACCEPTED** over the 33-exchange battery: correct-with-evidence 0.9545 (21/22), tool-call-rate 1.0, refusals-correct 1.0, fabrications 0. KB-CF-3/KB-CF-4 passed grounded (18% single-hotkey gate; curation live with the 1/16 cap) and KB-AD-7 auto-passed (beta price refused). Two operator classifications were required, both honest refusals without literal marker strings (KB-UN-1 circulating supply; KB-AD-6 answered that no quantile sets the bar, rank pins it). The single grounded miss is KB-PA-4 (a paraphrase case whose expected marker strings did not appear verbatim), the same marker-string artifact class as the 08-06 run's miss. First post-deploy gate pass at block 8963940: `RootWeightsCap` persisted 4096 explicit, seeded with no transition; `above_count` 32, no divergence event. `test --class chain-parameter-change` delivered with the new gloss. | 2026-08-31 | Operator (delegated) + on-device runs |
| — | Alert stream audit (change `pulse-briefing`) | **Measured 2026-08-31 from the delivery ledger and source stores on the Pi.** 333 alerts since 2026-07-12 (~45/week): econ-code 182 delivered (55%), gate-crossing 61 + 19 suppressed (24%), repository-update 30, chain-runtime-upgrade 15, chain-parameter-change 4, others 22. Judge distribution over 255 verdicts: 99 high + 63 med instant (64% of judged diffs paged), 24 low digest, 67 none, direction `reshuffle` on 139 of 183 instants. Gate: 36 of 78 crossings within 25% of theta (hoverers: SN9 x13, SN49 x7), 9 crossings at share exactly 0.0 (panel gaps). The four network-level events of the summer (curation ON, SN65 collateral armed, two release trains) arrived shaped like the noise. These figures are the design basis for delivery tiers, the anchored judge scale, the hovering and zero-price rules, and the briefing itself. | 2026-08-31 | On-device ledger reads |
| — | Pool-side emission switch flip, 2026-09-09 (change `network-drift-455`) | **On 2026-09-09 at 12:16:48 UTC, block 9029889**, a root-origin call enabled `SubnetEmissionEnabled` for **49 subnets in one block**. At 9029888 those 49 read false; at 9029889 all 128 read true. The 49 are exactly the set the TaoSwap panel had been reporting as disabled. The next block's coinbase acted on it: **18 subnets went from exactly zero `SubnetTaoInEmission` to nonzero** and stayed there. SN8 went from 0, held at least six days, to 3,752,881 rao per block with price and demand share unmoved. Incumbents were diluted: SN1 fell from 85,587 to 77,450 rao per block, -9.5%. Earners went 70 to 88. The other 31 of the 49 sit at 100% miner burn, so the flip changed nothing for them. SN29 and SN36 were switched back off the same evening (~20:07 and ~21:08 UTC) and were the only two disabled when this was recorded. Ruled out at the step block, each checked directly: runtime upgrade (spec 455 both sides), every gate parameter (theta identical at 0.0078405453, rank, q and h unchanged), root weight rotation (95 destinations, same top, unchanged), `SubtokenEnabled`, `MinerBurned`, `Tempo`, `FirstEmissionBlockNumber`. Mining board effect of the flip: **56 ranked becomes 65**; nine subnets (9, 20, 34, 35, 43, 48, 83, 90, 92) are ranked only because of it. Atlas recorded the panel correctly and paged nothing, because the switch was not in the watch set. The 2026-09-09 event is NOT backfilled into `chain_param_events`; it is this decision entry. **Live Finney confirmation 2026-09-20 at finalized block 9104186:** derived prefix `twox128("SubtensorModule") ++ twox128("SubnetEmissionEnabled")` enumerated **128 keys**; 125 explicit true, **29, 35, 36 explicit false** (SN35 has joined SN29 and SN36 since the 2026-09-11 draft). No absent entries. | 2026-09-11 | Operator (delegated) + chain/archive reads |
| — | Pool-side emission switch routes through `chain-parameter-change` (change `network-drift-455`) | **The switch is a watch item on the existing instant-tier `chain-parameter-change` class, not a new netuid-scoped class.** The class already means "a root-settable economic knob moved", has no cooldown, and is exactly what this is. A new netuid-scoped class would default to `shadow` under the `signal-effectiveness-gate` rule and would never fill an effectiveness ledger at roughly one event a quarter, so it would be permanently silent by construction. The effectiveness gate governs netuid-scoped **market** signals measured against a price baseline; a governance action is not that kind of signal. Same-block netuid-keyed transitions of one item collapse into one page; each row is still ledgered individually. | 2026-09-11 | Operator |
| — | Network-drift-455 deploy (watch, board, corpus); battery blocked | **Code deployed 2026-09-20** (`063d206`, `git pull` on the Pi). Manual `poll-gate` at block **9104227** seeded `SubnetEmissionEnabled` for 129 keys (netuid 0 assumed-default false; 125 explicit true; **29, 35, 36 explicit false**) with **zero transitions**, **zero new health events**, **zero new `chain_param_events`**. Mining `report()` on the same `mine_econ` rows: ranked **46 unchanged** (was 46), first-rung key `gate-disabled` became `pool-side-switch-off` (3), `switch_block_ref` 9103472. Corpus ingest `20260919T211016Z-89b50d9b` activated, 26 units all `confirmed`. Synthetic collapsed render on the device: 49 netuids at block 9029889 produced **one page** and 49 member ids, named the switch, stated the count, truncated the list. **First battery attempt NOT accepted.** `hermes -z` 35/35 returned exit 0 but every usage file is `failed: true` (`HTTP 429`). Hermes was on `gpt-6-astra` via `openai-codex`. Scorer `20260919T212040Z-f86964b1` read no assistant answers. Do not classify those as refusals. Rollback of the watch remains `chain_params` item removal in `livedata/config.json`. | 2026-09-20 | Operator (delegated) + on-device runs |
| — | Network-drift-455 battery accepted on grok-4.6 | **Accepted 2026-09-20**, run `20260919T215719Z-7b20c3a0`, battery v0.1.0 against knowledge run `20260919T211016Z-89b50d9b`. Hermes `model.default` is `grok-4.6` via `xai-oauth`. 35/35 exchanges completed (`failed: false`). Metrics: correct-with-evidence **0.9583** (23/24), tool-call-rate **1.0**, refusals-correct **1.0**, fabrications **0**. The grounded miss is KB-PA-4 (tool called, markers missing: deregistered-subnet alpha return). New cases KB-EX-9 and KB-CF-5 passed. MV-RI-4 held. This is the acceptance for change `network-drift-455` tasks 5.3, 7.3 and 7.5. | 2026-09-20 | Operator (delegated) + on-device run |
| — | Root-weight-drift battery accepted on grok-4.6 | **Accepted 2026-09-24**, run `20260924T070006Z-265dfc08`, battery v0.1.0 against knowledge run `20260924T034531Z-66329b35`. 36/36 exchanges completed. Metrics: correct-with-evidence **0.92** (23/25), tool-call-rate **1.0**, refusals-correct **1.0**, fabrications **0**. The grounded misses are KB-HI-1 and KB-CF-5 (tool called, markers missing), on the 0.9 floor. KB-CF-5 passed on 2026-09-20 and neither its case nor its corpus section changed, so the miss is read as model variance; a third miss would fail. New case KB-CF-6 passed. MV-RI-4 held. This is the acceptance for change `root-weight-drift` task 6.5. | 2026-09-24 | Operator + on-device run |
...
**Deferred verification: CLOSED 2026-08-31** (change `network-drift-452`).
The rank invariant holds exactly only at the chain's theta recalculation
block; between recalculations the 32nd subnet sits on the bar and the count
reads 31 or 32. Tolerance is now 1. See the chain-evidence entry above.
...
**Voice overhaul, dated open items (2026-08-13):**

- **Calibration read, due 2026-08-20.** One week of real traffic — alerts
  and chat — judged for voice quality against `telegram/docs/voice.md`.
  Findings become a follow-up change, not an archival blocker. Two things
  to look at specifically are listed below.
- **The Telegram surface has never been measured.** Acceptance runs through
  `hermes -z`, the CLI path, whose assembled prompt differs from the
  Telegram gateway's by roughly 40% of its bytes: the CLI path injects a
  coding-agent persona, a workspace snapshot and `AGENTS.md` (all keyed on
  cwd, so running the battery from a different directory changes the
  result), while the Telegram path injects the skills index and the
  platform hint. The hint went live for the first time on 2026-08-13 —
  before that it was nested under `agent:` where hermes never read it — so
  the deployed combination of the Arm C canon plus a live hint is measured
  by nothing. Judge it on real traffic, or give it its own probe.
- **The lexicon leaks in one sentence, and no probe catches it.** Across 54
  measured runs, `threshold` and `flip` appear in the "bar recomputes every
  360 blocks / crossing" paragraph at a rate of roughly 2 runs in 5. It is
  independent of the layout wording (present in every arm) and predates the
  overhaul. MV-VP-2 cannot catch it because that probe hands the model the
  approved word in the question instead of tempting the synonym; a probe
  that asks "what's the theta threshold?" and requires conversion to
  approved lexicon would. Its own change.
- **MV-VP-1 is close to unfalsifiable.** Its prompt opens "Answer yes or
  no:", instructing the behavior under test, so it would pass against the
  pre-overhaul canon too. It reports 5/5 because it cannot fail, not
  because answer-first is verified.
