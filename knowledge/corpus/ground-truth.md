...
## Root Reborn (spec 441, live 2026-08-03)

- Root Reborn changed how ROOT dividends are handled. It is LIVE on Finney, but its central feature is gated OFF, so read the two halves separately: what is already in force, and what is installed but dormant.
- IN FORCE now:
...
- INSTALLED BUT DORMANT: basket CURATION. Allocation follows each validator's root weight vector (`set_root_weights`, stored in `Weights[ROOT]`), but that setter ships DISABLED network-wide - `RootWeightSettingEnabled` is false on chain (verified 2026-08-06). While it is false every fund runs the NULL STRATEGY: dividends accumulate in place on the origin subnet. The stated reason for holding it back is to stop validators stampeding into TAO-cash (netuid 0) vectors on day one and recreating the old mechanical sell-pressure regime under a new name.
- Therefore: do NOT say root dividends are being curated, reinvested by strategy, or allocated across subnets. They are accumulating in place. The change that matters is governance flipping `AdminUtils::sudo_set_root_weight_setting_enabled` (or a migration in a later runtime); until then the null strategy is the observable baseline.
- `stake_into_basket` is gated as of spec 443, pending a state-growth review.
...
- "Root dividends are reinvested / curated into beta baskets across subnets" --> OUTDATED framing as of 2026-08-03 (spec 441). Root Reborn is live, but its curation setter (`set_root_weights`) ships disabled: `RootWeightSettingEnabled` is false, so every fund runs the null strategy and dividends accumulate in place. Only root-registered hotkeys earn root dividends (the rest is recycled) - that part IS in force.
