...
(2026-08-06, deployed): mainnet ran spec 441/442/443 and two chain changes
...
| `knowledge-base` | Done — 23 units active (all `confirmed`; the **2026-08-06 re-sync** brought the corpus from spec 440 to 443: the gate bar is rank-pinned and the quantile is inert, plus a Root Reborn section covering the live mechanism and its dormant curation gate), `atlas-kb` tools live in Hermes; benchmark re-run `20260806T080300Z-8cd094e9` perfect over the new corpus (20/20 grounded, 10/10 refusals, 0 fabrications — MV-RI-4 held), including the adversarial case that must refuse a live-sounding quantile |
