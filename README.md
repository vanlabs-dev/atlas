...
**Live chain (verified 2026-09-20, finalized ~9104590):** spec **467**.
...
128 keys, **29 / 35 / 36 off**, SN1 on; TaoSwap `emission_is_enabled`
matches. `RootWeightSettingEnabled` is **false** (unset since spec 464,
block 9088729); basket curation is off and `Weights[ROOT]` reads empty.
Corpus and this README are grounded at 467.

**On the device:** fleet clones + LAN boards at
`http://192.168.0.150:8480/` and `/mining.html`; hourly gate poll and
chain-parameter watch; Telegram pulse + shinogi.dev public edition.
No OpenSpec change is active. The 455 switch watch is deployed
(`063d206`). Pi HEAD `c6f8c46`.
...
| `knowledge-base` | Done — corpus grounded at Finney spec **467** (2026-09-20): pool-side switch distinct from the bar (off 29/35/36), `RootWeightSettingEnabled` false, rank-pinned bar. 35-exchange battery; last accepted run `20260919T215719Z-7b20c3a0` on `grok-4.6` (re-ingest on the Pi after this lock-in) |
...
`https://shinogi.dev` is the only public surface; everything else is LAN-only.
...
Q36–Q40. shinogi.dev is a public read, not that frontend.
