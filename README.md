...
`RootWeightSettingEnabled` is **false** (unset since spec 464, block
9088729; still unset). Basket curation is off. `BasketTradingEnabled` is
**true** (explicit). It gates `swap_basket` and is not curation.
...
| `knowledge-base` | Done — corpus grounded at Finney spec **469** (2026-09-23): pool-side switch distinct from the bar (off 29/35/36/108), `RootWeightSettingEnabled` false, rank-pinned bar. 35-exchange battery; last accepted run `20260919T215719Z-7b20c3a0` on `grok-4.6` (re-ingest on the Pi after this lock-in) |
...
Runtime upgrade rollout (change archived 2026-09-24, operator on the Pi):
run the dry run, enable the new timers, and retire the Hermes job. Steps
and one open live-drift finding (`RootWeightSettingEnabled` and
`RootWeightsCap` are gone at spec 469) are in
