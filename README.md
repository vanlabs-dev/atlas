...
## Current status (2026-09-20, chain at spec 455, corpus at spec 455)
...
third pass. The page was then redesigned: an amended `public-pulse` contract
allows a typeface and presentation script while still banning any browser
data fetch, so the edition now carries inline SVG charts drawn from recorded
series (the emission-gate bar, TAO, and demand share across all 128 subnets
with the rank-32 bar marked), a lede naming the largest recorded movement,
and attention rows whose reason is derived from the score components and
grouped, because 93 of 106 public rows carry one category and a fixed phrase
per category said nothing.
...
| `knowledge-base` | Done — corpus re-synced 2026-09-20 (change `network-drift-455`) from spec 452 to 455: pool-side emission switch as a distinct mechanism from the bar, `claim_root` admission (454), basket-escrow transfer rejection (453), registration queue (453), proxy call filters (453/454/455), crowdloan pallet as a filter fact. Battery extended with KB-EX-9 and KB-CF-5 (35 exchanges, 24 grounded). On-device acceptance `20260919T215719Z-7b20c3a0` on `grok-4.6`: correct-with-evidence 0.9583, refusals 1.0, tool-call-rate 1.0, 0 fabrications (KB-PA-4 marker miss only) |
...
| `shinogi-publish` | Done: deployed + archived (2026-09-11, change `shinogi-renderer`). `shinogi/atlas_shinogi.py` composes the public shinogi.dev edition from `livedata` and `fleet` rows, all opened read-only, to the `public-pulse` contract in `vanlabs-dev/shinogi`: five landmarks, gaps named never estimated, stale bounds per input (26h for the emission-gate bar; vitals dated, not stale for age; movers over the window since the previous publish). Charts are inline SVG from recorded series and introduce no figure the page does not report. Attention reuses `build_board` ordering, drops `pure_opaque`, caps at ten, and derives each reason from `div_signed`/`cold`/`econ_fresh`/`pulse_spike` rather than the near-constant category, grouping rows that share one; no score, glyph or thesis. Deltas compare against the last shinogi publish in its own `var/shinogi/shinogi.db`, never the Telegram watermark, and a zero delta is suppressed. Before any write the document is scanned for operator material and for any browser data fetch, failing the pass closed. The publish gate hashes the facts with the as-of line normalised out, and the checkout is fast-forwarded first (a diverged one fails closed). Own oneshot unit and timer at `00/6:55` local time, after the fleet pass. On the Pi: a write-scoped deploy key for the shinogi repo alone; Atlas itself still pulls anonymously over HTTPS. 70 tests green off-device |
...
**No change is active.** Every change is archived as of 2026-09-11:
`pulse-briefing`, `rotation-signal-gate` and `shinogi-renderer` closed that
day, joining `mining-triage` (2026-08-12) and `telegram-voice-overhaul`
(2026-08-13). `openspec/changes/` holds only `archive/`.

Atlas now has a public face. `https://shinogi.dev` serves a composed edition
every six hours from the `shinogi/` renderer, driven by
`atlas-shinogi.timer` on the Pi. That is the only outward-facing surface;
everything else stays LAN-only.

Two dated follow-ups in [docs/decisions.md](docs/decisions.md) are past due
and come before any new proposal:

1. **`mining.budget_band`, due 2026-08-21, three weeks overdue.** Still
   `null` in `fleet/config.json` while `mining.enabled` is `true`. The
   recorded rule: pick a hardware and capital band, or conclude mining is
   not being pursued and set `enabled` to `false` rather than maintain a
...
2. **Voice calibration read, due 2026-08-20.** One week of real Telegram
   ...
   path has never been measured directly; acceptance ran through the CLI.

The gate calibration read, also listed there, **closed 2026-09-11** with the
`pulse-briefing` acceptance.

Worth taking, not blocking:

- **The seven-day volume read on `rotation-signal-gate`, due 2026-09-16.**
  ...
  the honest read.
- **`network-drift-455` is deployed and the battery is accepted**
  (`20260919T215719Z-7b20c3a0` on `grok-4.6`). Ready to archive.
...
health fields, Q40 retention. Note that shinogi.dev is a *public read*, not
the operator monitoring frontend Phase 6 describes, and does not answer any
of those questions.
