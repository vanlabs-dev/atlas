...
| `knowledge-base` | Done — corpus re-synced 2026-09-20 (change `network-drift-455`) from spec 452 to 455: pool-side emission switch as a distinct mechanism from the bar, `claim_root` admission (454), basket-escrow transfer rejection (453), registration queue (453), proxy call filters (453/454/455), crowdloan pallet as a filter fact. Battery extended with KB-EX-9 (`claim_root` admission) and KB-CF-5 (zero TAO is the switch, not the bar); 35 exchanges, 24 grounded. Prior acceptance `20260831T165224Z-722feb65` over the 33-exchange battery remains the last on-device run until the new battery is accepted on the Pi |
