...
## Current status (2026-08-31, chain at spec 452)
...
**pulse-briefing** (2026-08-31, in progress): the alert stream becomes a
pulse. Every Telegram class carries a delivery tier (instant or briefing),
a daily/weekly briefing is composed from stores only, livedata persists an
hourly per-subnet panel snapshot and daily network vitals, the gate learns
hovering and zero-price rules, and the econ judge gets an anchored
significance scale. Investment/rotation tooling is Phase 7 territory.
...
| `knowledge-base` | Done — 23 units active (all `confirmed`; the **2026-08-31 re-sync** (change `network-drift-452`, pending device activation) brought the corpus from spec 443 to 452: single-hotkey 18% takeover gate, Root Reborn curation live under the 1/16 cap, on-chain beta pricing, spec 448 staking additions; the 2026-08-06 re-sync had brought the rank-pinned bar), `atlas-kb` tools live in Hermes; benchmark re-run `20260806T083003Z-58ae33ea` accepted over the new corpus (correct-with-evidence 0.95, refusals 10/10, 0 fabrications — MV-RI-4 held), including the adversarial case that must refuse a live-sounding quantile; the single grounded miss is a marker string artifact ("Jun" vs "june 2026"), not a knowledge gap |
