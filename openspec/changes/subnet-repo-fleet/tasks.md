...
- [~] 1.2 Operator gate: a **PROPOSED** entry recording the sample findings is drafted in `docs/decisions.md` — the operator ratifies it via the on-device discovery run (their action)
...
- [ ] 8.3 On-device acceptance on the Pi: run discovery + record the gate; seed a bounded first batch by hand; verify registry + a sample blobless clone + one recorded change range; install the timer; confirm convergence over the next passes; capture the result
