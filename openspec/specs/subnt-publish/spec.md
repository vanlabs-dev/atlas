# subnt-publish Specification

## Purpose
Composes the public subnt.dev edition from facts already recorded in the
Atlas stores as schema-valid data files and publishes them into the subnt
checkout, so that the page anyone can read carries only recorded, dated,
operator-free figures and names every input it does not have.
