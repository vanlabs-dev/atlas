...
## Purpose
Turn the subnet-repo fleet's read-only data into a rotation cockpit: an
emission-redirect map (how each subnet routes emissions, with `file:line`
evidence, or an opacity flag when economics load at runtime), epoch-scoped
repo-activity with a branch-pulse metric (so a live subnet working off its
default branch is not mistaken for abandoned), and alpha-price momentum reused
from the signals price panel — assembled into one explicitly-scored "attention
board" ranked from look-here to safely-ignore and rendered as a self-contained,
LAN-only static page. Read-only over the clones and store (additive tables
only), never executing subnet code, per-subnet fail-closed, and honest by
construction: unknown fractions, absent price history, and opaque economics are
shown as such rather than fabricated.
