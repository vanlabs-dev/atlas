...
## Purpose
Read-only code search over the subnet-repo fleet: one shared
(netuid, epoch)-scoped FTS index in the fleet store, kept in step by
reconcile (inline) and a standalone backfill, surfaced through the CLI
status block and the read-only atlas-fleet MCP server
(fleet_search / fleet_file / fleet_status) — never building, executing,
or mutating subnet code.
