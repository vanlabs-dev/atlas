# subtensor-repo-tracking Specification

## Purpose
Track the mainnet subtensor repository on the device: an identity-validated,
non-shallow local clone with a safe fail-preserving update process, recorded
change ranges, an incremental full-text index, and read-only Hermes tools
that answer repository questions with commit and file references and report
freshness honestly (PRD §12.6 ATLAS-REPO-001…009).
## Requirements
